...
# This script only differs in that the 'total' file is summarized without a super user!
...
total <- readRDS("total_percentage_samples_per_square2S.RDS")
...
# analysis script
# this script is used to carry out the analyses
# but the aggregated data are extracted from the following scripts
# -"temporal_samples_per_square.R"
# -"samples_per_square.R"
# -"iNEXT.R"
# -"bias_model.R"

# the goal here is to do some 'statistics' to say yes, difference or no no difference
...
# and 'temporal sampling' is repeated measures - one for each 2 week period
# ignore the various plotting stuff. I'll make all the plots look consistent for paper.
total <- readRDS("total_percentage_samples_per_square2.RDS")
...
ggsave("Figures/overall_sampling_super_user_removed.png", width=8.4, height=6.8, units="in")
...
#using a simple glm
glm1 <- glm(no_samples ~ experimental_group, offset=log(no_cells+1),
            family="poisson",data=total)
summary(glm1)
#spillover effect

#include no_cells as an offset term
glm1 <- glm(no_samples ~ status + experimental_group:status, offset=log(no_cells+1),
            family="poisson",data=total)
summary(glm1)

#standard treatment contrasts - each experiment group is compared to control group
...
ggplot(coefs)+
  geom_pointrange(aes(x=level, y=estimate, 
                      ymin=estimate-(2*std_error), ymax=estimate+(2*std_error),
                      colour=group))+
  geom_hline(yintercept=0)+
  scale_color_viridis_d("Compared to control") +
  theme_classic()+
...
#or user-defined contrasted to test:

#(1) control vs mean experiment
#(2) experiment type 1 vs experiment type 2

levels(total$experimental_group)
contrasts(total$experimental_group) <- matrix(c(-2, 1, 1, 0, -0.5, 0.5), ncol = 2)
#intercept is not the grand mean
#the coefficients below are contrasts (1) and (2) described above
...
ggplot(coefs)+
  geom_pointrange(aes(x=level, y=estimate, 
                      ymin=estimate-(2*std_error), ymax=estimate+(2*std_error)))+
  geom_hline(yintercept=0)+
  scale_color_viridis_d("Contrast") +
  theme_classic()+
  facet_wrap(~group)+
  ylab("Difference in sampling rate")+
  coord_flip() 
...
ggsave("Figures/model_results.png", width=7.4, height=6.8, units="in")
...
ggsave("Figures/sampling_percentages_temporal.png", width=7.6, height=8.7, units="in")
...
ggsave("Figures/spatial_bias.png", width=7.8, height=8.5, units="in")
...
ggsave("Figures/spatial_bias_statistical_test.png", width=6.7, height=7.5, units="in")

# as above, I think two different comparisons are worthwhile
# 1) "control" vs experimental grouped
# 2) control vs experimental vs experimental with a leaderboard
...
#############################################################
# iNext sampling data
# So I don't think any 'stats' are necessary for this one
# and in fact I don't know how much to even include this one
# the two 'control' groups have higher accumulation after than before
# so this might not be the best way to show the data.
# because in part this is dependent on 'how well' the notion of how we 
# assessed value was done.
# but regardless, it is potentially an interesting figure to include and maybe add to the story a little bit
inext_sampling <- readRDS("lga_inext_data.RDS")


ggplot() + 
  geom_line(data=inext_sampling, aes(x=x, y=y, group=interaction(status, type_of_curve), color=status, linetype=type_of_curve), size=1) + 
  labs(linetype = "") +
  theme_minimal() + 
  xlab("Number of records") + 
  ylab("Number of species") +
  facet_wrap(~ lga, ncol=2, scales="free") + 
  xlim(0, 30000)
